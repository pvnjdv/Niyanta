import { WorkflowEdge, WorkflowNodeInstance } from '../types/workflow.types';
...
export function buildAgentWorkflowDefinition(agent: AgentBlueprint, agentId: string): {
  workflowId: string;
  name: string;
  description: string;
  ...
  const llmId = `${agentId}_llm`;
  const notifyId = `${agentId}_notify`;

  return {
    workflowId,
    name: `${agent.name} Agent Workflow`,
    description: `Workflow backing the ${agent.name} agent`,
    nodes: [
      {
      ...
      },
      {
        ...
        config: { prompt: agent.systemPrompt },
      ...
      },
      {
        ...
        name: 'Result Output',
        config: { channel: 'internal', message: 'Agent execution complete' },
        position: { x: 700, y: 200 },
      },
    ],
    edges: [
      {
        id: `e_${triggerId}_${llmId}`,
        fromNodeId: triggerId,
        toNodeId: llmId,
      },
      {
        id: `e_${llmId}_${notifyId}`,
        fromNodeId: llmId,
        toNodeId: notifyId,
      },
    ],
...
export function buildAgentCanvasLayout(agent: AgentBlueprint, workflowId: string): Array<Record<string, unknown>> {
  ...
  return [
    ...
    },
    {
      ...
      x: 320,
      y: 80,
      inputInfo: `Evaluate risk, confidence, and whether ${agent.name} can proceed autonomously.`,
    },
    {
      id: workflowBlockId,
      blockType: 'workflow',
      refId: workflowId,
      name: `${agent.name} Core Workflow`,
      category: 'AI Agent',
      color: agent.color,
      x: 620,
      y: 80,
      inputInfo: `Execute the reusable ${agent.name.toLowerCase()} workflow with modular steps.`,
    },
    {
      id: auditId,
      blockType: 'node',
