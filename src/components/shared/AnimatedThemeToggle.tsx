...
const AnimatedThemeToggle: React.FC<AnimatedThemeToggleProps> = ({ theme, onToggle, className }) => {
...
const SolarSwitch: React.FC<{ isDark: boolean }> = ({ isDark }) => {
  const duration = 0.7;
  ...
  useEffect(() => {
    const controlsSun = motionAnimate(scaleSun, isDark ? 0 : 1, { duration });
    const controlsMoon = motionAnimate(scaleMoon, isDark ? 1 : 0, { duration });
    return () => {
      controlsSun.stop();
      controlsMoon.stop();
    };
