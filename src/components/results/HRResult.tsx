...
import { HRResult as HRResultType, HRTask } from '../../types';

interface HRResultProps {
  result: HRResultType;
}

type ChecklistTab = 'before_day_one' | 'day_one' | 'week_one' | 'month_one';

const HRResult: React.FC<HRResultProps> = ({ result }) => {
  const [activeTab, setActiveTab] = useState<ChecklistTab>('before_day_one');
  ...
  const taskRowStyle = (isCritical?: boolean): React.CSSProperties => ({
    display: 'flex',
    alignItems: 'flex-start',
    gap: 10,
    padding: '8px 0',
  ...
  const accessChipStyle = (level: string): React.CSSProperties => {
  ...
  const buddyCardStyle: React.CSSProperties = {
    backgroundColor: 'var(--accent-dim)',
    border: '1px solid var(--border)',
    borderRadius: 4,
    padding: '12px 14px',
  ...
  const buddyTitleStyle: React.CSSProperties = {
    fontFamily: "'DM Sans', sans-serif",
    fontWeight: 600,
    fontSize: 13,
    color: 'var(--text-primary)',
  ...
  const buddyTextStyle: React.CSSProperties = {
    fontFamily: "'DM Sans', sans-serif",
    fontSize: 12,
    color: 'var(--text-muted)',
  };
  ...
  const currentChecklist: HRTask[] = result.checklist?.[activeTab] || [];
