...
import { InvoiceResult as InvoiceResultType } from '../../types';

interface InvoiceResultProps {
  result: InvoiceResultType;
}

const InvoiceResult: React.FC<InvoiceResultProps> = ({ result }) => {
  ...
  const getSeverityType = (severity: string): 'danger' | 'warning' | 'success' | 'muted' => {
    switch (severity) {
      case 'HIGH':
        return 'danger';
      case 'MED':
  ...
  const getComplianceIcon = (checkResult: string): string => {
    switch (checkResult) {
      case 'PASS':
        return '✓';
      case 'FAIL':
  ...
  const getComplianceType = (checkResult: string): 'success' | 'danger' | 'warning' | 'muted' => {
    switch (checkResult) {
      case 'PASS':
        return 'success';
      case 'FAIL':
